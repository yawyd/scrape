...
def follow(user_id=''):
    # even if you already follow this user.py a successful message is returned
    return pinterest.follow_user(user_id=user_id)


def unfollow(user_id=''):
    # even if you don't follow this user.py a successful message is returned
    return pinterest.unfollow_user(user_id=user_id)


def get_following_batched(username=None, max_items=500):
    # you can get following on any user.py, default is current user.py
...
def get_following(username=None):
    # Gets full following list of a user.py
...
def get_followers(username=None):
    # Gets a full list of user.py followers
...
def search(max_items=100, scope='boards', query='food'):
    ...
    # Instead you need to search for something else and extract the user.py data from there.
...
def invite(board_id='', target_user_id=''):
    # If user.py is already invited to the board, you get 403 error.
    return pinterest.invite(board_id=board_id, user_id=target_user_id)


def delete_invite(board_id='', target_user_id=''):
    # If user.py is not invited to the board, you get 403 error.
